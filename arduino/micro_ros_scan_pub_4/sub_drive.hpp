...
void pid_control(float tgtSpeedR, float tgtSpeedL, int cntR, int cntL){
  //R wheel
  float speedR = float(cntR - prevCountR) / DURATION; //現在の回転速度（カウント/秒）
  ...
  if(dutyR > 0){ //正方向トルク
    ledcWrite(8, constrain(abs(dutyR), 0, 255));    //255以下に制限
  ...
  //L wheel
  float speedL = float(cntL - prevCountL) / DURATION;
  errP = tgtSpeedL - speedL;
  errIL += (errP * DURATION);
  errD = (errP - errPrevL) / DURATION;
  float dutyL = Kp * errP + Ki * errIL + Kd * errD;
  if(dutyL > 0){ //正方向トルク
    ledcWrite(9, constrain(abs(dutyL), 0, 255)); //255以下に制限
...
void set_motors(){
  //motor settings
  ledcSetup(8, PWM_FREQ, 8);    //PWM幅分解能8bitで指定
  ledcAttachPin(mtRPwmPin, 8);  //mtRPwmPinピンにチャネル0を設定
  ledcSetup(9, PWM_FREQ, 8);    //PWM幅分解能8bitで指定
  ledcAttachPin(mtLPwmPin, 9);  //mtLPwmPinピンにチャネル1を設定
